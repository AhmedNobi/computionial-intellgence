...
class ReadData:
    ...
    def discriminative_line(self,X1, X2,class_, feature_, w1=0, w2=0, b=0):
        # get 2 inputs w1, w2 & b
        # put getMin(X1) and estimate x2
        # put getMax(X1) and estimate x1
        # Draw Graph
        # Draw Line
        lineX = [float(min(X1)), float(max(X1))]
        lineY = [-(b + w1 * xi) / w2 for xi in lineX]
        fig = plt.figure('Discrimination Line')
        plt.scatter(X1[0:20], X2[0:20], label=self.class_label(class_[0]))
        plt.scatter(X1[20:40], X2[20:40], label=self.class_label(class_[1]))
